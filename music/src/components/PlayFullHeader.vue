<template>
  <div class="header">
    <div v-if="!isShowLyric" :class="{playing : !paused}" id="turnbj"></div>
    <div class="back" @click="$emit('showPlayBar')">
      <svg
        t="1608190197397"
        class="icon"
        viewBox="0 0 1024 1024"
        version="1.1"
        xmlns="http://www.w3.org/2000/svg"
        p-id="3165"
        width="200"
        height="200"
      >
        <path
          d="M702.94996 958.89419c-8.375761 0-16.751522-3.196809-23.14514-9.590426L265.159102 534.658046c-12.787234-12.777001-12.787234-33.513278 0-46.289256L679.80482 73.722048c12.787234-12.787234 33.502022-12.787234 46.289256 0 12.787234 12.777001 12.787234 33.513278 0 46.289256L334.593498 511.512906l391.501602 391.501602c12.787234 12.777001 12.787234 33.513278 0 46.289256C719.701482 955.697381 711.325721 958.89419 702.94996 958.89419z"
          p-id="3166"
          fill="#ffffff"
        ></path>
      </svg>
    </div>
    <div class="message">
      <h6>{{ currentMusica.name }}</h6>
      <p>{{ currentMusica.song.artists[0].name }}</p>
    </div>
    <div class="share">
      <svg
        t="1608190375790"
        class="icon"
        viewBox="0 0 1024 1024"
        version="1.1"
        xmlns="http://www.w3.org/2000/svg"
        p-id="4030"
        width="200"
        height="200"
      >
        <path
          d="M794.7 461.3c-9 0-16 7.8-14.9 16.7 1.4 11.2 2.1 22.5 2.1 34 0 35.1-6.7 69.3-19.8 101.6-13.6 33.5-33.5 63.5-59.3 89.3s-55.8 45.7-89.3 59.3C581.2 775.4 547 782 512 782c-35.1 0-69.3-6.6-101.6-19.8-33.5-13.6-63.5-33.5-89.3-59.3-25.8-25.8-45.7-55.8-59.3-89.3C248.6 581.3 242 547.1 242 512s6.6-69.3 19.8-101.6c13.6-33.5 33.5-63.5 59.3-89.3 25.8-25.8 55.8-45.7 89.3-59.3C442.7 248.7 476.9 242 512 242c14.3 0 28.5 1.1 42.5 3.3 9.1 1.4 17.2-5.6 17.2-14.8 0-7.3-5.3-13.7-12.6-14.8-91.5-14.5-188.4 13.5-258.9 83.8-117.4 117-117.5 307.4-0.3 424.6s307.1 117.2 424.3 0c68.2-68.2 96.7-161 85.5-249.8-1-7.5-7.4-13-15-13z"
          fill="#ffffff"
          p-id="4031"
        ></path>
        <path
          d="M776.7 233.3h-141c-8.3 0-15 6.7-15 15s6.7 15 15 15h104.4L517.6 485.7c-5.9 5.9-5.9 15.4 0 21.2 5.9 5.9 15.4 5.9 21.2 0l222.8-222.8v104.2c0 8.3 6.7 15 15 15s15-6.7 15-15v-140c0.1-8.3-6.7-15-14.9-15z"
          fill="#ffffff"
          p-id="4032"
        ></path>
      </svg>
    </div>
  </div>
</template>

<script>
export default {
  props: ['paused','currentMusica','isShowLyric']
};
</script>

<style lang="less">
.header {
  width: 100%;
  height: 80px;
  display: flex;
  align-items: center;
  padding: 7px;
  position: relative;
  #turnbj {
    position: absolute;
    top: 100%;
    left: 50%;
    margin-left: -24px;
    // background: chartreuse;
    z-index: 18;
    display: block;
    content: "";
    width: 96px;
    height: 137px;
    background-image: url("https://s3.music.126.net/mobile-new/img/needle-ip6.png?be4ebbeb6befadfcae75ce174e7db862=");
    background-size: contain;
    background-repeat: no-repeat;
    transform-origin: top left;
    transform: rotate(-15deg);
    transition: 1s all;
  }
  .playing{
    transition: 1s all;
    transform: rotate(0deg) !important;
  }
  .back {
    svg {
      width: 25px;
      height: 25px;
    }
  }
  .message {
    flex: 1;
    color: white;
    h6 {
      width: 60%;
      margin-left: 20%;
      display: -webkit-box;
      -webkit-box-orient: vertical;
      -webkit-line-clamp: 1;
      overflow: hidden;
      text-overflow: ellipsis;
      // text-align: left;
    }
    p {
      font-size: 12px;
      color: hsla(0, 0%, 100%, 0.6);
      // text-align: left;
    }
  }
  .share {
    svg {
      margin-top: 10px;
      width: 35px;
      height: 35px;
    }
  }
}

</style>